...
- **Сокращение пространств имён материалов.** `materialNamePrefixesToStrip` по умолчанию равно `["minecraft:"]`; префикс скрывается только в названии, а события остаются в списке материалов, scan, aggregate, просмотре события и его соседей. Для полного имени задайте `"materialNamePrefixesToStrip": []` в `config.json` и перезапустите сервер.
