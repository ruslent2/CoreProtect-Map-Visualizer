...
- **Сокращение пространств имён материалов.** Используются только явно заданные в `materialNamePrefixesToStrip` префиксы; отсутствующее или некорректное значение нормализуется в `[]`. Префикс скрывается только в названии, а события остаются в списке материалов, scan, aggregate, просмотре события и его соседей.
